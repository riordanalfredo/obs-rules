...
router.post('/addsourceSession', (req, res, next) => {
  ...
  const dtsession_string = {
    name: req.body.name,
    id_session: req.body.id_session,
    id_datatype: req.body.id_datatype,
    id_session_datatype: req.body.id_datatype_session,
  };
  ...
  con.query(
    'INSERT INTO datatype_session SET ?',
    dtsession_string,
    (err, res1) => {
      if (err) throw err;
    }
  ...
  con.query(
    'SELECT * from datatype_session WHERE id_session = ?',
    [req.body.id_session],
    (err, rows) => {
      if (err) throw err;
...
router.post('/datasession', (req, res, next) => {
  ...
  // Get mysql client from the connection pool
  con.query(
    'SELECT * FROM datatype_session WHERE id_session = ? AND id_datatype = ? ORDER BY id_datatype ASC',
    dtsession_string,
    (err, rows) => {
      if (err) throw err;
...
router.post('/updateSourceSession', (req, res, next) => {
  ...
  if (req.body.status == 1) {
    ...
    var string_query =
      'UPDATE datatype_session SET start_capture = ?, status = ? WHERE id_session = ? AND id_datatype = ? AND id_session_datatype = ? ';
    var session_vars = [
      new Date(),
      req.body.status,
      req.body.id_session,
      req.body.id_datatype,
      req.body.id_datatype_session,
    ];
    con.query(string_query, session_vars, (err, result) => {
      ...
      con.query(
        'SELECT * FROM datatype_session WHERE id_session = ? ORDER BY id_datatype ASC',
        [req.body.id_session],
        (err, rows) => {
          if (err) throw err;
    ...
    var string_query =
      'UPDATE datatype_session SET end_capture = ?, status = ? WHERE id_session = ? AND id_datatype = ? AND id_session_datatype = ?';
    var session_vars = [
      new Date(),
      req.body.status,
      req.body.id_session,
...
router.post('/deletesourcesession', (req, res, next) => {
  ...
  const query_string1 =
    'DELETE FROM datatype_session WHERE datatype_session.id_datatype = ? AND datatype_session.id_session_datatype = ? AND datatype_session.id_session = ? ';
  con.query(
    query_string1,
    [req.body.id_datatype, req.body.id_session_datatype, req.body.id_session],
    (err, result) => {
      if (err) throw err;
...
router.post('/updateempatica', (req, res, next) => {
  ...
  // SQL Query > Select Data
  var string_query =
    'UPDATE datatype_session SET id_empatica = ? WHERE id_session = ? AND id_datatype = ? AND id_session_datatype = ? ';
  var session_vars = [
    req.body.serial,
    req.body.id_session,
    req.body.id_datatype,
    req.body.id_session_datatype,
  ];
  con.query(string_query, session_vars, (err, result) => {
    if (err) throw err;
    console.log(`Changed ${result.changedRows} row(s)`);
