...
const mqtt_url = url.parse(
  process.env.CLOUDMQTT_URL ||
    'mqtt://wfejcfvu:t7Os7ERNBJ0s@m14.cloudmqtt.com:19641'
);
...
router.get('/all/:id_session', (req, res, next) => {
  ...
  con.query(
    'SELECT * FROM rules WHERE id_session = ? ORDER BY type ASC;',
...
const roles = (id_session, callback) => {
  ...
  con.query(
    'SELECT * FROM object_session WHERE id_session = ? ORDER BY id ASC;',
    [id_session],
    (err, rows) => {
      if (err) throw err;
      rows.forEach((row) => {
...
router.post('/validateRule/:rulesID', (req, res, next) => {
  ...
  if (rule[0].magnitude == 'Proximity') {
    ...
    for (var i = 0; i < actions.length && find != true; i++) {
      var point = {};
      console.log(
        'The second action selected: ',
        rule[0].second_action,
        actions[i].action_desc
      );
      ...
      if (rule[0].magnitude == 'Sequence' || rule[0].magnitude == 'Time') {
        ...
        if (
          actions[i].action_desc != null &&
          actions[i].action_desc == rule[0].second_action
        ) {
    ...
    for (var i = 0; i < actions.length && find2 != true; i++) {
      ...
      if (rule[0].magnitude == 'Sequence' || rule[0].magnitude == 'Time') {
        if (
          actions[i].action_desc != null &&
          actions[i].action_desc == rule[0].first_action
        ) {
          console.log('Si  entro second validation');
          find2 = true;
          point['id'] = actions[i].id;
          point['content'] =
            'Perform this actions is important' + actions[i].action_desc;
    ...
    if (find == true && find2 == true) {
      if (rule[0].magnitude == 'Time') {
        ...
        if (difSeconds > parseInt(rule[0].value_of_mag) * 60) {
          rulesValidated['title'] =
            rule[0].feedback_wrong +
            '<span style="color:orange">' +
            ' Time response: ' +
            parseFloat(difSeconds / 60).toFixed(2) +
            '</span>';
        ...
        if (positionActionA < positionActionB) {
          ...
          pointMessage['className'] = 'feedbackok';
          pointMessage['start'] = end;
          pointMessage['content'] =
            'Well done the team perform this critical action';
        } else {
          rulesValidated['title'] =
            rule[0].feedback_wrong +
            '. Next time consider the order of the actions';
          point['className'] = 'negative';
          rulesValidated['status'] = 'wrong';
          point['start'] = end;
          point['end'] = start;
    ...
    console.log(
      'This is the last point of the array ####: ',
      rulesValidated['points']
    );
  }
...
router.post('/updatetypeRules', (req, res, next) => {
  ...
  con.query(
    'UPDATE object_session SET name = ? WHERE id = ?',
    dtobj_string,
    (err, result) => {
      if (err) throw err;
...
router.post('/addRule', (req, res, next) => {
  ...
  var rule_string = {
    name: req.body.name,
    type: req.body.typeRule,
    id_session: req.body.sessionid,
    id_first_act: req.body.firstAction,
    id_second_act: req.body.secondAction,
    magnitude: req.body.magnitude,
    value_of_mag: req.body.value,
    feedback_ok: req.body.feedbackCorrect,
    feedback_wrong: req.body.feedbackWrong,
  };
  con.query('INSERT INTO rules SET ?', rule_string, (err, result) => {
    if (err) throw err;

    con.query(
      'SELECT * FROM rules WHERE id_session = ? ORDER BY id ASC;',
      [req.body.sessionid],
      (err, rows) => {
        if (err) throw err;
        rows.forEach((row) => {
...
router.post('/delete', (req, res, next) => {
  ...
  con.query(
    'DELETE FROM rules WHERE rules.id= ?',
    [req.body.id_rule],
    (err, rows) => {
      if (err) throw err;
