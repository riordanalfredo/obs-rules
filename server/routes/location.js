...
const mqtt_url = url.parse(
  process.env.CLOUDMQTT_URL ||
    'mqtt://wpancwwq:JM7WMMC9MqzM@m14.cloudmqtt.com:11474'
);
    ...
    mqtt_client.subscribe('TOPIC', function () {
      ...
      mqtt_client.on('message', function (topic, message, packet) {
        ...
        if (messageJson.success == true) {
          location_string = {
            id_tag: messageJson.tagId,
            x: messageJson.data.coordinates.x,
            y: messageJson.data.coordinates.y,
            z: messageJson.data.coordinates.z,
            timestamp: new Date(messageJson.timestamp * 1000),
            acc: messageJson.data.acceleration,
...
router.post('/insertLocationTag', (req, res, next) => {
  ...
  // Get mysql client from the connection pool
  con.query(
    'INSERT INTO location_data_session SET ?',
    location_string,
    (err, res1) => {
      if (err) throw err;
