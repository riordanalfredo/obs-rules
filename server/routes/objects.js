...
router.post('/countobjssession', (req, res, next) => {
  ...
  con.query(
    'SELECT count(*) as count FROM object_session where id_object = ? and id_session = ?',
    objString,
    (err, rows) => {
      if (err) throw err;
...
router.post('/addobjsession', (req, res, next) => {
  ...
  const dtsession_string = {
    id_object: req.body.id_obj,
    id_session: req.body.id_session,
    name: req.body.name,
  };

  con.query(
    'INSERT INTO object_session SET ?',
    dtsession_string,
    (err, res1) => {
      if (err) throw err;
    }
...
router.get('/studentsession/:ids', (req, res, next) => {
  ...
  con.query(
    'SELECT * FROM object_session WHERE id_session = ? ORDER BY name ASC;',
    [req.params.ids],
    (err, rows) => {
      if (err) throw err;
...
router.post('/trackers/', (req, res, next) => {
  ...
  if (req.body.type == 'pozyx') {
    con.query(
      'SELECT * FROM trackers WHERE type = ?',
      [req.body.type],
      (err, rows) => {
        if (err) throw err;
    ...
    con.query(
      'SELECT * FROM datatype_session WHERE id_session = ? AND id_datatype = 3;',
      [req.body.id_session],
      (err, rows) => {
        if (err) throw err;
...
router.post('/updateserial', (req, res, next) => {
  ...
  if (req.body.type == 'pozyx') {
    con.query(
      'UPDATE object_session SET serial = ?, type = ? WHERE id = ? AND id_session = ?',
      dtobj_string,
      (err, result) => {
        if (err) throw err;
    ...
    con.query(
      'UPDATE object_session SET empatica = ?, type = ? WHERE id = ? AND id_session = ?',
      dtobj_string,
      (err, result) => {
        if (err) throw err;
        console.log(`Changed ${result.changedRows} row(s)`);
...
router.post('/updateobjcoordinates', (req, res, next) => {
  ...
  con.query(
    'UPDATE object_session SET coordinates = ? WHERE id = ?',
    dtobj_string,
    (err, result) => {
      if (err) throw err;
      console.log(`Changed ${result.changedRows} row(s)`);
...
router.post('/deleteobjectsession', (req, res, next) => {
  ...
  const query_string1 =
    'DELETE FROM object_session WHERE object_session.id = ?';
  con.query(query_string1, [req.body.id_objectsession], (err, result) => {
    if (err) throw err;
