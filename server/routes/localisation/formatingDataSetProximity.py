...
def formatExcel(df, session):
    ...
    df['timestamp'] = df['timestamp'].apply(
        pd.to_datetime, myFormat, errors='ignore')
    df['phase'] = '2'
    df['quartile'] = '2'
...
def formatJson(df, session):
    ...
    roll = []
    for item in df['data']:
        x.append(item.get("coordinates").get("x"))
        y.append(item.get("coordinates").get("y"))
        z.append(item.get("coordinates").get("z"))
        aX.append(item.get("acceleration").get("x"))
        aY.append(item.get("acceleration").get("y"))
        aZ.append(item.get("acceleration").get("z"))
        pitch.append(item.get("orientation").get("pitch"))
        yaw.append(item.get("orientation").get("yaw"))
        roll.append(item.get("orientation").get("roll"))

    df.insert(0, "x", x, True)
    df.insert(1, "y", y, True)
    df.insert(2, "z", z, True)
    df.insert(3, "accX", aX, True)
    df.insert(4, "accY", aY, True)
    df.insert(5, "accZ", aZ, True)
    df.insert(6, "pitch", pitch, True)
    df.insert(7, "yaw", yaw, True)
    df.insert(8, "roll", roll, True)
...
def cleaning(df):
    df = df[['session', 'timestamp', 'tracker', 'x', 'y', 'z', 'pitch',
             'yaw', 'roll', 'accX', 'accY', 'accZ']]  # Re-order dataframe columns
    # order session (1 - 12)
    df = df.sort_values(['tracker', 'session', 'timestamp'])
...
def normalization(df):
    ...
    session_tracker = df.groupby(['session', 'tracker']).size(
    ).reset_index().rename(columns={0: 'count'})

    for index, row_pair in session_tracker.iterrows():
        # GET sub dataframe for a particular tracker
        session_df = df[(df['tracker'] == row_pair['tracker'])
                        & (df['session'] == row_pair['session'])]
        #print (session_df)
        ...
        session_df = session_df.set_index(
            'timestamp')  # need to index timestamp
        #merge = session_df.merge(df_time.to_frame(),left_on='timestamp', how='right')
        merge = pd.merge(df_time.to_frame(), session_df,
...
def filteringPhases(df, phase1, phase2):
    ...
    filtered = df[(df['timestamp'] >= pd.to_datetime(phase1[0]).strftime(myFormatB)) & (
        df['timestamp'] <= pd.to_datetime(phase2[0]).strftime(myFormatB))]
...
def filteringPhasesAdding(df, phase1, phase2):
    ...
    filtered = df[(df['timestamp'] >= phase1.strftime(myFormatB))
                  & (df['timestamp'] <= phase2.strftime(myFormatB))]
...
def asign_phases(df, phase1, phase2):
    ...
    if (phases is None):
        ...
        for x in range(1, len(df_filtered.to_numpy()[0])-1):
            ...
            initialTimeOpt = pd.to_datetime(
                array_filtered[x]).strftime(myFormatB)
            endTimeOpt = pd.to_datetime(
                array_filtered[x + 1]).strftime(myFormatB)
            #initialTime = array_filtered[x]
            #endTime = array_filtered[x+1]
            phase = 'phase ' + str(x)
            print(initialTime, endTime, phase)
            df.loc[((df['timestamp'] >= initialTime) & (df['timestamp'] < endTime)) | (
                (df['timestamp'] >= initialTimeOpt) & (df['timestamp'] < endTimeOpt)), 'phase'] = phase
            #df3 = df
...
def addingRoles(df, phase):
    ...
    print(phase)
    row_added = {'tracker': 'PTN', 'session': df.iloc[0]['session'], 'phase': phase, 'quartile': df.iloc[0]
                 ['quartile'], 'timestamp': df.iloc[0]['timestamp'], 'x': ref_patient().get('x'), 'y': ref_patient().get('y')}
