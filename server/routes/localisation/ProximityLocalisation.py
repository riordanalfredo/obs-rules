...
def main():
    ...
    #print(typeOfGraph, phase1, phase2, centeredRole, len(C), roles, session);
    initAnalisis(file, centeredRole, proxemic, phase1,
                 phase2, roles, typeOfGraph, session, idRule)
...
def initAnalisis(file, centeredRole, proxemic, phase1, phase2, roles, typeOfGraph, session, idRule):
    ...
    df_proxemic_labels, prox_labels = distances.proxemicsLabels(
        df_distancesBetTrackers, n)
    ...
    if (typeOfGraph == 'full'):
        ...
        graph, message = vis.graphDefinition(
            dfnorm, trackers_names, 'porcentages')
        # print(graph)
        name = vis.visualiseGraph1(
            graph, session, 'porcentages', proxemic, idRule)
