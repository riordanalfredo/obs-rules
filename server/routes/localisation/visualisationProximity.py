...
def aggregateLabels(df, prox_columns):
    df3 = pd.DataFrame(
        {'proxemic': ['False', 'personal', 'intimate', 'social', 'public']})
...
def orderTrackers(role, df_trackers):
    ...
    df_shifted = pd.concat(
        [df_trackers.iloc[[index-1], :], df_trackers.drop(index-1, axis=0)], axis=0)
...
def graphDefinition(df, df_trackers, typ):
    ...
    for i in range(0, len(values)-1):
        ...
        if i > 0:
            ...
            message += ' '+roles[i] + ' spent ' + color + values[i] + \
                '%'+' </span>' + 'of their time with ' + roles[0] + ' \n'
    return g, message
...
def visualiseGraph(g, session, phase, type, proxemic):
    ...
    g.es["color"] = ["pink" if float(
        proxLabel) < 0.5 else "blue" for proxLabel in g.es["proxLabel"]]
    visual_style = {}
...
def visualiseGraph1(g, session, type, proxemic, idRule):
    ...
    g.es['label'] = g.es['proxLabel']
    g.vs['color'] = 'light blue'
    g.es["color"] = ["pink" if (proxLabel != None) and float(
        proxLabel) < 0.5 else "blue" for proxLabel in g.es["proxLabel"]]
...
def normalizedata(df_labels):
    ...
    if (minimo == maximo):
        ...
        for x in range(0, len(array_filtered)):
            y = (((int(array_filtered[x])) -
                 float(minimo)) / (float(maximo - minimo)))
