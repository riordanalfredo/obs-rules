...
var fs = require('fs');
const { dbInfo } = require('./db-constants');
...
router.post('/newaction', (req, res, next) => {
  var results = [];
  var action_string = {
    name: req.body.newactionname,
    action_type: req.body.newactiontype,
  };
  //}
  ...
  con.query('INSERT INTO actions SET ?', action_string, (err, result) => {
    ...
    const query_string2 =
      'SELECT * FROM actions WHERE action_type = "event" OR action_type = "critical" ORDER BY name ASC;';
...
router.post('/allactionssession', (req, res, next) => {
  const results = [];
  const query_string =
    'SELECT action_session.id, action_session.id_action, action_session.action_desc, actions.action_type FROM action_session, actions WHERE action_session.id_action=actions.id AND (actions.action_type = "event" OR actions.action_type = "critical") AND action_session.id_session = ? order by action_session.action_desc ASC;';
...
router.post('/allactionswithobjects', (req, res, next) => {
  const results = [];
  const query_string =
    'SELECT actions.id, actions.name, actions.action_type, action_session.id_object, action_session.time_action FROM actions left join action_session on (actions.id=action_session.id_action) and action_session.id_session = ? order by actions.name ASC;';
  con.query(query_string, [req.body.id_session], (err, rows) => {
    if (err) throw err;
...
router.post('/updatenotes', (req, res, next) => {
  const results = [];
  con.query(
    'UPDATE action_session_object set notes = ? WHERE action_session_object.id = ?',
    [req.body.notes, req.body.id_actionsessionobject],
    (err, rows) => {
      if (err) throw err;
...
router.post('/getnotebyid', (req, res, next) => {
  const results = [];
  const query_string =
    'SELECT action_session_object.notes FROM action_session_object WHERE action_session_object.id = ? ;';
...
router.post('/insertactionsession', (req, res, next) => {
  ...
  var action_string = {
    ...
    action_desc: req.body.desc,
    time_action: new Date(),
  };
  //}

  con.query(
    'INSERT INTO action_session SET ?',
    action_string,
    (err, result) => {
      if (err) throw err;
      console.log(`Inserted ${result.affectedRows} row(s)`);
...
router.post('/addactionsectionobject', (req, res, next) => {
  ...
  var action_string = {
    id_session: req.body.id_session,
    id_actionsession: req.body.id_actionsession,
    id_action: req.body.id_action,
    action_desc: req.body.desc,
    id_object: req.body.id_object,
    time_action: new Date(),
  ...
  con.query(
    'INSERT INTO action_session_object SET ?',
    action_string,
    (err, result) => {
      if (err) throw err;
      console.log(`inserted ${result.affectedRows} row(s)`);
...
router.post('/allactionsinDB', (req, res, next) => {
  const results = [];
  const query_string =
    'SELECT actions.id, actions.name, actions.action_type order by actions.name ASC;';
  con.query(query_string, [req.body.id_session], (err, rows) => {
    if (err) throw err;
...
router.post('/actionsessionexists', (req, res, next) => {
  ...
  //const exists = 1;
  const query_string =
    'SELECT * FROM action_session where action_session.id_action = ? and action_session.id_session = ?;';
  con.query(
    query_string,
    [req.body.id_action, req.body.id_session],
    (err, rows) => {
      if (err) throw err;
...
router.post('/addactiontothissession', (req, res, next) => {
  ...
  var action_string = {
    id_session: req.body.id_session,
    id_action: req.body.id_action,
    action_desc: req.body.action_name,
  };
...
router.post('/deleteactionsession', (req, res, next) => {
  ...
  const query_string1 =
    'DELETE FROM action_session WHERE action_session.id = ?';
  con.query(query_string1, [req.body.id_actionsession], (err, result) => {
    if (err) throw err;
...
router.post('/addactionobject', (req, res, next) => {
  ...
  con.query(
    'SELECT count(*) as total from action_session WHERE id_session = ? AND id_action = ? ORDER BY id ASC',
    [req.body.id_session, req.body.id_action],
    (err, result) => {
      if (err) throw err;
      ...
      con.query(
        'UPDATE action_session SET id_object = ? WHERE id_session = ? AND id_action = ?',
        obj_string,
        (err, result) => {
          if (err) throw err;
      ...
      const query_string =
        'SELECT actions.id, actions.name, actions.action_type, action_session.id_object, action_session.time_action FROM actions left join action_session on (actions.id=action_session.id_action) and action_session.id_session = ?';
      con.query(query_string, [req.body.id_session], (err, rows) => {
        if (err) throw err;
...
router.get('/getactionobjects/:id_session', (req, res, next) => {
  const results = [];
  con.query(
    'SELECT * from action_session WHERE id_session = ? ORDER BY id ASC',
    [req.params.id_session],
    (err, rows) => {
      if (err) throw err;
...
router.post('/deleteaction', (req, res, next) => {
  ...
  con.query(query_string1, [req.body.id_action], (err, result) => {
    if (err) throw err;
    console.log(`Deleted ${result.affectedRows} row(s)`);
...
router.post('/deleteactionobject', (req, res, next) => {
  ...
  const query_string1 =
    'DELETE FROM action_session_object WHERE action_session_object.id = ?';
  con.query(query_string1, [req.body.id_actionsessionobject], (err, result) => {
    if (err) throw err;
    console.log(`Deleted ${result.affectedRows} row(s)`);
