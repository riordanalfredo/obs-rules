...
router.post('/generateJson2', (req, res, next) => {
  ...
  for (var i = 0; i < dataActions.length; i++) {
    ...
    if (dataActions[i].duration != null && dataActions[i].name != null) {
      ...
      if (dataActions[i].action_desc == 'Start CPR') {
        ...
        if (
          dataActions[i].action_type == 'critical' &&
          dataActions[i].name == 'PTN'
        ) {
          var ct = {};
          ...
          critical_item['content'] =
            '<img src="../../../img/warning.png" style="width: 40px; height: 40px;"><div class="special-time">' +
            moment(dataActions[i].duration, 'hh:mm:ss.SSS').format('mm:ss') +
            '</div><div id="text">' +
            dataActions[i].action_desc +
            '</div>';
          //  }
          ...
          if (dataActions[i].action_desc == 'Deliver Shock') {
            item['className'] =
              'action shock ' + dataActions[i].object_type.toLowerCase();
          } else {
            item['className'] =
              'action ' + dataActions[i].object_type.toLowerCase();
          }
  ...
  for (var i = 0; i < CPR_array.length; i++) {
    ...
    item['content'] =
      moment(CPR_array[i].duration, 'hh:mm:ss.SSS').format('mm:ss') +
      '<div id="text">Compressions</div>';
    ...
    for (var j = 0; j < dataActions.length; j++) {
      if (
        dataActions[j].action_desc == 'Stop CPR' &&
        dataActions[j].id == nearCPR
      ) {
        item['end'] = dataActions[j].time_action;
        item['title'] =
          diff_minutes(dataActions[j].duration, CPR_array[i].duration) +
          ' mins';
      }
    }
  ...
  fs.writeFile(
    'client/data/session_' + id_session + '.json',
...
router.post('/generateJson', (req, res, next) => {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (data[i].param_value == 'alert') {
      ...
      var critical_item = {};
      time_from_start = data[i].duration
        .split(':')
        .slice(-2)
        .join(':')
        .split('.')[0];
      ...
      if (data[i].action_desc.split(' ')[0] == 'Ask') {
        critical_item['content'] =
          ...
          time_from_start +
          '</div><img src="../../../img/ask.png" style="width: 136px; height: 112px;">';
      } else if (data[i].action_desc.split(' ')[0] == 'Lose') {
        critical_item['content'] =
          '<div class="special-time">' +
          time_from_start +
          '</div><img src="../../../img/lose.png" style="width: 136px; height: 112px;">';
      }
    ...
    if (data[i].duration != null && data[i].name != null) {
      ...
      if (data[i].action_desc == 'Start CPR') {
        ...
        item['content'] =
          time_from_start + '<div id="text">' + data[i].action_desc + '</div>';
        if (data[i].name != null) {
  ...
  for (var i = 0; i < CPR_array.length; i++) {
    var id_reg = CPR_array[i].id;
    var item = {};
    time_from_start = CPR_array[i].duration
      .split(':')
      .slice(-2)
      .join(':')
      .split('.')[0];
    ...
    for (var j = 0; j < data.length; j++) {
      if (data[j].action_desc == 'Stop CPR' && data[j].id == nearCPR) {
        ...
        item['title'] =
          diff_minutes(data[j].duration, CPR_array[i].duration) + ' mins';
      }
    }
    participants[CPR_array[i].name].push(item);
    //console.log(item);
...
router.get('/getJsonFromFile', (req, res, next) => {
  ...
  var obj = JSON.parse(
    fs.readFileSync('client/data/session_' + id_session + '.json', 'utf8')
  );
...
router.get('/bringGraph/:idRule', (req, res, next) => {
  ...
  let validate =
    req.query.id_session + '_' + id_rule + '_' + 'porcentages_personal.png';
  ...
  const logOutputError = (name) => (data) =>
    ...
    (err, rows) => {
      ...
      if (exist == true) {
        ...
        con.query(query_string, [id_rule], (err, rows) => {
          ...
          return res.json({
            path: './data/graphs/' + validate,
            rule: results,
            message: result[0].feedback_wrong,
          });
        });
      } else {
        var query_string =
          'select aobj.time_action from action_session_object as aobj where (aobj.id_action=? and aobj.id_session=?) OR (aobj.id_action=? and aobj.id_session=?) ORDER BY aobj.time_action asc;';
        con.query(
          query_string,
          [
            results[0].id_first_act,
            req.query.id_session,
            results[0].id_second_act,
            req.query.id_session,
          ],
          (err, rows) => {
            if (err) throw err;
            ...
            getRoles(results[0].id_session, (resultsRoles) => {
              ...
              pythonProcess.stdout.on('data', logOutput('stdout'));
              /*          pythonProcess.stderr.on('data', (data) => {
              console.log('Error? ',data.toString());
          });*/

              pythonProcess.stderr.on('data', logOutputError('stderr'));
